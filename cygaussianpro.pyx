...
def GaussianElimination(np.ndarray nested_fractions):
    cdef int i, j, k, n
    # cdef Fraction inner_temp, temp
    ...
    for i in range(n-1):            # loop to perform the gauss elimination
        ...
        for k in range(i+1, n):
            ...
            for j in range(n+1):
                # nested_fractions[k][j] = nested_fractions[k][j] - temp*nested_fractions[i][j] # make the elements below the pivot elements equal to zero or eliminate the variables
    ...
    for i in range(n-1, -1, -1): # back-substitution
        ...
        for j in range(i+1, n): 
            if j!=i: # then subtract all the lhs values except the coefficient of the variable whose value is being calculated
                # aug_matrix[i] = aug_matrix[i] - nested_fractions[i][j]*aug_matrix[j]
                aug_matrix[i] -= nested_fractions[i][j]*aug_matrix[j]
        aug_matrix[i] /= nested_fractions[i][i] # now finally divide the rhs by the coefficient of the variable to be calculated
